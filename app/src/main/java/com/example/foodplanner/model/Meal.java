...
public class Meal {
    ...
    public List<IngredientItem> getIngredientItemList() {
        ...
        // Add ingredients and measures (up to 20, as per TheMealDB API)
        for (int i = 1; i <= 20; i++) {
            try {
                String ingredient = (String) this.getClass().getMethod("getStrIngredient" + i).invoke(this);
                String measure = (String) this.getClass().getMethod("getStrMeasure" + i).invoke(this);
                if (ingredient != null && !ingredient.isEmpty()) {
                    String imageUrl = "https://www.themealdb.com/images/ingredients/" + ingredient + "-Small.png";
                    ingredients.add(new IngredientItem(ingredient, measure, imageUrl));
                }
            } catch (Exception e) {
                // Skip if method doesn't exist or ingredient is null
            }
        }
        return ingredients;
    }
